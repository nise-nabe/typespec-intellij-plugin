...
Pinned IDE build is **`262.10968.63`** (IntelliJ IDEA 2026.2.3; `gradle/libs.versions.toml` → `intellij-idea`). IPGP 2.x sandboxes use a **product-version directory** (e.g. `.intellijPlatform/sandbox/plugin/IU-2026.2.3/log/idea.log`), not the build number and not always `plugin/build/idea-sandbox/.../system/log/idea.log` (Platform Gradle Plugin 1.x layout).
