  ...
  if ! "${gh_source}" auth status >/dev/null 2>&1; then
    ...
    if [[ -n "${token}" ]]; then
      if ! "${gh_source}" auth login --hostname github.com --git-protocol https --with-token <<<"${token}"; then
        ...
        echo "Use ManagePullRequest for PRs, or set GH_TOKEN in your cloud environment's secrets." >&2
      fi
    else
      echo "Warning: gh is not authenticated and GH_TOKEN/GITHUB_TOKEN is unset." >&2
      echo "Set GH_TOKEN in your cloud environment's secrets, or use the ManagePullRequest tool for PRs." >&2
    fi
  fi
